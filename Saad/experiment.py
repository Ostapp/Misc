...
def save_products_links(search_string):
	...
	if file_name[0] not in os.listdir('/home/ostap/Documents/programming/Saad/products_lists/'):
...
def load_final_results():
	...
	try:
		...
		if 'final_sellers_list_ebay' in os.listdir('.'):
...
def remove_duplicates():
	with open ('final_sellers_list_ebay', 'r') as f:
		results1 = pickle.load(f)
		print str(len(results1))
		indexes = len(results1)
		for i in range(indexes-2):
			print i
			try:
				for j in range(indexes)[i+1:]:
					if results1[i]['seller-name'] == results1[j]['seller-name']:
						print results1[j]['seller-name']
						del results1[j]
			except IndexError as e:
				print e
				continue
		with open ('final_sellers_list_ebay', 'w') as f:
